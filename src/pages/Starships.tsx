...
const Starships = () => {
  ...
  useEffect(() => {
    const fetchStarships = async () => {
      try {
        const data: Starship[] = await api.getStarships();
        setStarships(data);
      } catch (error) {
  ...
  const visibleStarships = starships.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  const totalPages = Math.ceil(starships.length / ITEMS_PER_PAGE);
        ...
        <table className='w-full min-w-[500px] text-left text-gray-100 text-sm sm:text-base rounded-xl'>
          ...
          <tbody>
            {visibleStarships.map((ship) => (
              ...
              >
                <td className='px-6 py-4 font-medium text-indigo-100 text-center '>{ship.name}</td>
                <td className='px-6 py-4 text-indigo-100 text-center '>{ship.model}</td>
                <td className='px-6 py-4 text-indigo-100 text-center '>{ship.manufacturer}</td>
                <td className='px-6 py-4 text-indigo-100 text-center '>{ship.starship_class}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className='sm:hidden flex flex-col gap-6 mt-6'>
        {visibleStarships.map((ship) => (
          ...
          >
            ...
            <p className='text-indigo-100 text-lg font-bold mb-3'>{ship.name}</p>

            <p className='text-indigo-300 font-semibold text-sm mb-1'>Model</p>
            <p className='text-indigo-100 mb-3'>{ship.model}</p>

            <p className='text-indigo-300 font-semibold text-sm mb-1'>Manufacturer</p>
            <p className='text-indigo-100 mb-3'>{ship.manufacturer}</p>

            <p className='text-indigo-300 font-semibold text-sm mb-1'>Class</p>
            <p className='text-indigo-100'>{ship.starship_class}</p>
          </div>
        ))}
      </div>
