...
const Card = ({ image, name, description, link = "#" }: CardProps) => {
  return (
    <Link
      to={link}
      className='group relative block rounded-2xl overflow-hidden w-full max-w-xs mx-auto shadow-xl hover:shadow-indigo-500/40 transition-shadow duration-300'
      style={{ minHeight: 380 }}
    >
      <img
        alt={name}
